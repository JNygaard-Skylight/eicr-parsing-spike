...
<!--
  ...
  The following parameters are available
    ...
    attributes "true" | "false"  (Autodetecting) Use only when the schema has no attributes as the
context nodes
    ...
    terminate= yes | no | true | false | assert  Terminate on the first failed assertion or successful
report
                                         Note whether any output at all is generated depends on the XSLT implementation.

  The following parameters can be specified as Schematron variables in diagnostics, assertions and so
on.
 ...
 Conventions: Meta-stylesheets that override this may use the following parameters
    ...
    full-path-notation = 1|2|3  select the notation for the full paths: 1=computer, 2=human,
3=obsolescent
...
<!--
  VERSION INFORMATION
   ...
   Version: 2008-07-14 update for XSLT2 and inclusion experiments
   		...
   		* RJ For path generation, test for //iso:schema not just /iso:schema, for potential embedded
Schematron support
...
-->
  ...
  <!-- DPC set to true if contexts should be checked on text nodes nodes (if only-child-elements is
  false)
...
-->
  ...
  If the params are set explictly the above may be used, and also either if
      @*
      @*|node()
   in all cases the result may not be equivalent, for example if you specify no attributes and the
  schema
...
-->
  ...
  <!-- DPC if this is set to
    ...
    '//' select all contexts with // a key rather than walking the tree explictly in each mode (XSLT2
  only)
-->
  ...
  <xsl:template match="/">
    <xsl:if test="//schold:*[ancestor::iso:* or descendant::iso:*]">
      ...
      <xsl:message><xsl:call-template name="outputLocalizedMessage">
  ...
  <!-- Default uses XSLT 1 -->
  <xsl:template
    match="iso:schema[not(@queryBinding) or @queryBinding='xslt'
  ...
  <xsl:template match="iso:schema" priority="-1">
    <xsl:message terminate="yes"><xsl:call-template name="outputLocalizedMessage">
  ...
  <xsl:template match="*" mode="stylesheetbody">
   ...
   for ZIP archives.
    ...
    <xsl:call-template
      name="iso:exslt.add.imports" /> <!-- RJ moved report BH -->
	<axsl:param name="archiveDirParameter" />
	<axsl:param
      name="archiveNameParameter" />
	<axsl:param name="fileNameParameter" />
	<axsl:param
      name="fileDirParameter" />
	...
	<xsl:apply-templates
      mode="do-types" select="xsl:import-schema" />
    <xsl:text>&#10;&#10;</xsl:text><xsl:comment>KEYS AND FUNCTIONS</xsl:comment><xsl:text>&#10;</xsl:text>
	<xsl:apply-templates
      mode="do-keys" select="xsl:key | xsl:function " />
  ...
  <xsl:template name="generate-default-rules">
    <xsl:text>&#10;&#10;</xsl:text>
		...
		<xsl:comment>This mode can be used to generate an ugly though full XPath for locators</xsl:comment><xsl:text>&#10;</xsl:text>
   		<axsl:template
      match="*" mode="schematron-select-full-path">
      <xsl:choose>
        ...
        <xsl:when test=" $full-path-notation = '3' ">
          <!-- Obsolescent. Use for humans, but no good for paths unless namespaces are known
          out-of-band -->
					<axsl:apply-templates select="."
		...
		<xsl:comment>This mode can be used to generate an ugly though full XPath for locators</xsl:comment><xsl:text>&#10;</xsl:text>
   		<axsl:template
      match="*" mode="schematron-get-full-path">
	...
	<xsl:text>&#10;</xsl:text>
    <!--simplify
    the error messages by using the namespace prefixes of the
     instance rather than the generic namespace-uri-styled qualification-->
	<axsl:template
      match="node() | @*" mode="schematron-get-full-path-2">
      <!--report
    ...
    the error messages by using the namespace prefixes of the
     instance rather than the generic namespace-uri-styled qualification-->
	<axsl:template
      match="node() | @*" mode="schematron-get-full-path-3">
      <!--report
      the element hierarchy-->
      <axsl:for-each select="ancestor-or-self::*">
        <axsl:text>/</axsl:text>
    ...
    <!-- use this if you need generate ids in separate passes,
		...
		<axsl:template
      match="/" mode="generate-id-from-path" />
      ...
      <axsl:value-of select="concat('.text-', 1+count(preceding-sibling::text()), '-')" />
		...
		<axsl:template
      match="comment()" mode="generate-id-from-path">
      <axsl:apply-templates select="parent::*" mode="generate-id-from-path" />
      <axsl:value-of select="concat('.comment-', 1+count(preceding-sibling::comment()), '-')" />
    </axsl:template>
		<axsl:template
      match="processing-instruction()" mode="generate-id-from-path">
      <axsl:apply-templates select="parent::*" mode="generate-id-from-path" />
      <axsl:value-of
      ...
      <axsl:value-of select="concat('.@', name())" />
		...
		<axsl:template match="*"
      mode="generate-id-from-path" priority="-0.5">
      <axsl:apply-templates select="parent::*" mode="generate-id-from-path" />
      <axsl:text>.</axsl:text>
      <!--
			<axsl:choose>
...
-->
	...
	<axsl:template
      match="/" mode="generate-id-2">U</axsl:template>
    ...
    <xsl:if test="not(@pattern)">
      <xsl:message><xsl:call-template name="outputLocalizedMessage">
          <xsl:with-param name="number">5</xsl:with-param>
                ...
                <xsl:if
      test="not(../../iso:pattern[@id = current()/@pattern])
                and not(../../iso:include)">
      <xsl:message><xsl:call-template name="outputLocalizedMessage">
  ...
  <xsl:template match="iso:extends">
    <xsl:if test="not(@rule)">
      <xsl:message><xsl:call-template name="outputLocalizedMessage">
     	...
     	<xsl:if
      test="not(//iso:rule[@abstract='true'][@id= current()/@rule] )">
      <xsl:message>
        <xsl:call-template name="outputLocalizedMessage">
  ...
  <!-- ISO INCLUDE -->
  <!-- This is only a fallback. Include really needs to have been done before this as a separate
  pass.-->
  ...
  <xsl:template match="iso:include">
    <xsl:variable name="document-uri" select="substring-before(concat(@href,'#'), '#')" />
       <xsl:variable
      name="fragment-id" select="substring-after(@href, '#')" />
      ...
      <xsl:when test="string-length( $fragment-id ) &gt; 0">
        <xsl:variable name="theDocument_1" select="document( $document-uri,/ )" />
              <xsl:variable
          name="theFragment_1" select="$theDocument_1//iso:*[@id= $fragment-id]" />
              <xsl:if
          test="not($theDocument_1)">
      ...
      <xsl:otherwise>
        <!-- Import the top-level element if it is in schematron namespace,
              ...
              <xsl:variable
          name="theFragment_2" select="$theDocument_2/iso:*" />
              <xsl:variable
          name="theContainedFragments" select="$theDocument_2/*/iso:*" />
              <xsl:if
          ...
          <xsl:message><xsl:call-template name="outputLocalizedMessage">
              <xsl:with-param name="number">21</xsl:with-param>
       		...
       		<xsl:apply-templates
          select="$theFragment_2 | $theContainedFragments " />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
      ...
      <xsl:when test="string-length( $fragment-id ) &gt; 0">
        <xsl:variable name="theDocument_1" select="document( $document-uri,/ )" />
          ...
          <xsl:message><xsl:call-template name="outputLocalizedMessage">
              ...
              <xsl:apply-templates
          select=" $theFragment_1" mode="do-all-patterns" />
      </xsl:when>
  ...
  <xsl:template match="iso:let">
    <xsl:if test="ancestor::iso:schema[@queryBinding='xpath']">
	  ...
	  <xsl:if
      test="ancestor::iso:schema[@queryBinding='xpath2']">
      <xsl:message><xsl:call-template name="outputLocalizedMessage">
      ...
      <!-- Extension to allow validation in multiple documents -->
        ...
        <xsl:otherwise>
          <axsl:variable name="thePath"
            ...
            <axsl:choose>
              <axsl:when
                test="starts-with( ., 'http:') or starts-with(., 'file:' )
						   or starts-with(., '/')"><!--
                  ...
                  <xsl:text>//(</xsl:text>
                    ...
                    <xsl:text>(</xsl:text>
                      ...
                      test="position()!=last()">|</xsl:if>
							...
							<xsl:text>)</xsl:text>
							<xsl:if
                    test="$visit-text='false'">[not(self::text())]</xsl:if>
                </xsl:attribute>
              </axsl:apply-templates>
            </axsl:for-each>
          </xsl:when>
  ...
  <xsl:template match="iso:rule[not(@abstract='true')] ">
    ...
    <!-- DPC priorities count up from 1000 not down from 4000 (templates in same priority order as
    before) -->
      ...
      <xsl:if
        test="not($select-contexts) and not(starts-with(tokenize(@context, '/')[last()], '@'))">
        <axsl:apply-templates select="{$context-xpath}" mode="M{count(../preceding-sibling::*)}" />
      </xsl:if>
    </axsl:template>
  </xsl:template>
  ...
  <xsl:if
      test="string-length(normalize-space($start)) > 0">
      <xsl:text> </xsl:text>
     <xsl:apply-templates
  ...
  allow a fallback -->
  <!--xsl:template
  name="handle-namespace" version="2.0">
   <xsl:namespace name="{@prefix}" select="@uri">
</xsl:template-->

  <xsl:template name="handle-namespace">
    <!-- experimental code from
    http://eccnet.eccnet.com/pipermail/schematron-love-in/2006-June/000104.html -->
      ...
      <xsl:when use-when="not(element-available('xsl:namespace'))"
        ...
        <xsl:variable name="ns-dummy-elements">
          <xsl:element name="{@prefix}:dummy" namespace="{@uri}" />
       	   ...
       	   <xsl:variable
          name="p" select="@prefix" />
           <xsl:copy-of
          select="exsl:node-set($ns-dummy-elements)
  ...
  <xsl:template name="process-assert">
    ...
    <!-- "Rich" parameters -->
      ...
      <xsl:with-param name="role" select="$role" />
		...
		<xsl:if
      test=" $terminate = 'yes' or $terminate = 'true' ">
      <axsl:message terminate="yes">TERMINATING</axsl:message>
    </xsl:if>
	    <xsl:if
      test=" $terminate = 'assert' ">
      <axsl:message terminate="yes">TERMINATING</axsl:message>
    </xsl:if>
      ...
      <xsl:otherwise>
        <xsl:variable name="localizationDocumentFilename">
          <xsl:value-of select="concat('sch-messages-', $langCode, '.xhtml')" />
				...
				<xsl:variable
          name="theLocalizedMessage">
          <xsl:value-of
            select='document( $localizationDocumentFilename, /)//xhtml:p[@id=concat("sch-message-", $number, "-", $langCode)]/text()' />
        </xsl:variable>
  ...
  <xhtml:div class="ErrorMessages">
    <!-- Where the error message contains dynamic information, the message has been split into an
    "a" and a "b" section.
	     This has been done even when the English does not require it, in order to accomodate different
    language grammars
    ...
    <xhtml:p id="sch-message-3a">Fail: This implementation of ISO Schematron does not work with
      schemas using the query language </xhtml:p>
    ...
    <xhtml:p id="sch-message-15">Schema error: The key element is not in the ISO Schematron
      namespace. Use the XSLT namespace.</xhtml:p>
    ...
    <xhtml:p id="sch-message-28">Schema implementation error: This schema has abstract patterns, yet
      they are supposed to be preprocessed out already</xhtml:p>
